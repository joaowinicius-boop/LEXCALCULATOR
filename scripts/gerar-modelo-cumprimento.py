...
P.append(para([run('Conforme sentença/acórdão, a parte Executada foi condenada a indenizar a parte Exequente em danos morais, no valor de ((MORAL_ORIG)).', False)]))
P.append(para([run('Já no que diz respeito aos danos materiais, fora determinada a devolução em dobro dos descontos realizados pela Exequente.', False)]))
P.append(para([run('Os honorários advocatícios foram arbitrados ao percentual de ((HON_PCT)) sobre o valor da condenação.', False)]))
